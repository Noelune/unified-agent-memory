...
- Run the unified-memory Python suite, the new focused Node tests, and the Web profile rebuild. Refresh the existing `<local-web-url>` GUI and verify both settings entries plus a non-clipped left-footer popup.
