...
If `<opencodego-repo>` remains a linked non-repository directory, do not force a repository initialization; report its changed files and retain the source change for the active Web profile link.
...
Refresh `<local-web-url>` and verify:
