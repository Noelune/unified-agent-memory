...
const SECRET_PATTERNS = [
  /((?:api[_-]?key|access[_-]?token|auth[_-]?token|token|secret|password)\s*[:=]\s*)[^\s`'"\\]{4,}/gi,
...
function redactOutput(value) {
  ...
  text = text.replace(SECRET_PATTERNS[0], '$1<REDACTED>')
...
function boundOutput(value) {
  ...
  const marker = '\n[output truncated by dsh-unified-agent-memory]\n'
  return `${text.slice(0, MAX_OUTPUT - marker.length)}${marker}`
}
function resolveConfig(config = {}) {
  ...
  const remoteEnabled = parseBoolean(config.remoteEnabled, false)
...
function runCore(cfg, args) {
  return new Promise((resolvePromise) => {
    ...
    execFile(
      ...
      { env, timeout: TIMEOUT_MS, maxBuffer: MAX_OUTPUT * 2, windowsHide: true },
      (err, stdout, stderr) => {
        if (err?.code === 'ERR_CHILD_PROCESS_STDIO_MAXBUFFER' && stdout) {
          resolvePromise({ ok: true, output: boundOutput(stdout) })
          return
        }
        if (err) {
          const detail = boundOutput(String(stderr || err.message || '').trim().split('\n').slice(-3).join('\n'))
          resolvePromise({
            ok: false,
            output: '',
...
function renderText(_args, value) {
  return [{ type: 'text', text: boundOutput(value.output ?? value) }]
}
...
export function apply(ctx, config = {}) {
  ...
  if (!ctx?.tools || typeof ctx.tools.register !== 'function') return { cfg, runCore }
  ...
  if (typeof ctx.inject !== 'function') return { cfg, runCore }
  ctx.inject(['webServer'], (hostCtx) => {
    ...
    host.effect?.(() => {
      ...
      if (typeof server.on === 'function') {
        ...
        return () => {
          if (typeof server.off === 'function') server.off('request', handle)
          else if (typeof server.removeListener === 'function') server.removeListener('request', handle)
        }
      }
      return undefined
